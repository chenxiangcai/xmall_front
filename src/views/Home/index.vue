        ...
        <section class="w mt30 clearfix" v-if="item.type === 2 ">
          <MShelf :title="item.name">
            <div slot="content" class="hot">
              <MGoods :goods="o" v-for="(o,i) in item.panelContents" :key="i"></MGoods>
            </div>
          </MShelf>
        </section>
        ...
        <section class="w mt30 clearfix" v-if="item.type === 3 ">
          <MShelf :title="item.name">
            <div slot="content" class="floors">
              ...
              <MGoods :goods='o' v-for='(o,i) in item.panelContents' :key='i' v-if='o.type===0'></MGoods>
            </div>
          </MShelf>
        </section>
...
import MGoods from '@/components/MallGoods'
// 主要逻辑
export default {
  data () {
    return {
  ...
  async created () {
    ...
    if (res.code === 200) {
      this.homeList = res.result // 所有模块
      console.log(this.homeList)
      const { panelContents } = res.result.find(item => item.type === 0) // 轮播图模块
